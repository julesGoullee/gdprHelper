;(function () {
  ...
  function elClick(
    selector,
    callback,
    { targetParent = false, maybeTimer = false } = {}
  ) {
  ...
  registerCookie(
    'cabinet_bedin_cookies=eyJ3ZWJzaXRlIjpmYWxzZSwiZ29vZ2xlIjpmYWxzZSwieW91dHViZSI6ZmFsc2UsInZpbWVvIjpmYWxzZSwiZmJfcGl4ZWwiOmZhbHNlLCJzb2NpYWxzIjpmYWxzZX0=:'
  )
  registerCookie('notice_gdpr_prefs=0:')

  // quantcast
  new MutationObserver(kickQuantcast).observe(document.body, {
    childList: true,
  })
    ...
    try {
      ...
      if (!!window.Didomi && !!window.didomiOnReady) {
        ...
        window.didomiOnReady.push(function (Didomi) {
          if (Didomi.notice.isVisible() && !!Didomi.setUserDisagreeToAll) {
            ...
            document.querySelectorAll('#didomi-host').forEach(function (d) {
      ...
      if (window.ct_ultimate_gdpr_cookie) {
        ...
        elClick('#ct-ultimate-gdpr-cookie-change-settings', () => {
          elClick(
            '.ct-ultimate-gdpr-cookie-modal-slider [for="cookie0"]',
            () => {
      ...
      if (!!window.evidon && !!window.evidon.barrier) {
        ...
        elClick(
          '.evidon-barrier-cookiebutton',
          () => {
            console.error(
              "[extension:Never-Consent] Sadly I can't do more for you, I just can display to you the right modal."
            )
            clearInterval(kick)
          },
          { maybeTimer: 2000 }
      ...
      if (!!window.evidon && !!window.evidon.banner) {
        ...
        console.error(
          '[extension:Never-Consent] Evidon banner still not managed'
        )
      }
      ...
      if (!!window.Sddan && window.Sddan.cmpLoaded) {
        ...
        elClick('#sd-cmp #main_body button:nth-child(2)', () => {
          elClick('#sd-cmp #details_body + div button', () =>
            clearInterval(kick)
          )
        })
      }
        ...
        if (window.orejime.manager) {
          ...
          elClick('.orejime-Button--info', () => {
            Object.keys(orejime.internals.manager.consents).forEach(
              (ele) => (orejime.internals.manager.consents[ele] = false)
            )
      ...
      if (window.theShield) {
        ...
        if (switchBoxes.length) {
          switchBoxes.forEach((checkbox) => {
            if (checkbox.checked) {
              checkbox.click()
            }
          })

          elClick('.slshield-info__cta.slshield-info__cta-accept', () =>
            clearInterval(kick)
          )
        }
      }
      ...
      if (!!window.SibboCMP && !!SibboCMP.isOpen()) {
        log('Sibbo')
        elClick(
          'sibbo-cmp-layout .sibbo-panel__aside a.sibbo-cmp-button[data-nav="purposes"]',
          () => {
            elClick(
              'sibbo-cmp-layout a.sibbo-cmp-button[data-button-reject-all][data-left="next"]',
              () => {
                elClick('sibbo-cmp-layout a#purposesNavLegInt', () => {
                  elClick(
                    'sibbo-cmp-layout a.sibbo-cmp-button[data-button-reject-all][data-left="save-and-exit"]',
                    () => {
                      elClick(
                        'sibbo-cmp-layout a#purposesLegIntSaveAndExit',
                        () => clearInterval(kick)
                      )
                    }
                  )
      ...
      if (
        !!window.appconsent &&
        document.getElementById('appconsent') != null &&
        !!appconsent.default &&
        !!appconsent.default.isInitialised
      ) {
      ...
      if (
        ...
        !!_iub.cs &&
        !!_iub.cs.options &&
        !!_iub.cs.options.callback &&
        !!_iub.cs.api &&
        _iub.cs.api.consentGiven
      ) {
        log('iubenda')
        _iub.cs.options.callback.onBannerShown = () =>
          _iub.cs.api.consentGiven('rejectButtonClick')
        clearInterval(kick)
      }
      // LiveRamp
      if (window.__tcfapi) {
      ...
      console.error(
        '[extension:Never-Consent] encountered a problem, please open an issue here https://github.com/MathRobin/Never-Consent/issues'
      )
